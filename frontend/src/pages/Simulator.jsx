...
const Simulator = () => {
  ...
  const handleManualSubmit = async (e) => {
    ...
    setResult(null);
    try {
      // If high risk is checked, we can inject some high V features on the frontend 
      // or the backend will handle it if we send something.
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      <header className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Transaction Simulator</h1>
          <p className="text-slate-400 mt-1">Generate real-time transactions to test the ML detection engine</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass p-6 h-full flex flex-col">
            <h3 className="font-bold flex items-center gap-2 mb-6">
              {mode === 'auto' ? <RefreshCcw className="w-4 h-4 text-primary" /> : <CreditCard className="w-4 h-4 text-primary" />}
              {mode === 'auto' ? 'Incoming Data Stream' : 'New Transaction Details'}
            </h3>
            ...
            {mode === 'auto' ? (
              <div className="flex-1 flex flex-col justify-center gap-4">
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 relative overflow-hidden">
                  ...
                  <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-1">Payload Status</p>
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${result ? 'bg-safe shadow-lg shadow-safe/50' : 'bg-slate-700'}`}></div>
                    <span className="text-sm font-mono">{result ? 'TRANSACTION_RECEIVED' : 'AWAITING_INPUT...'}</span>
                  </div>
                </div>
              ...
              <form onSubmit={handleManualSubmit} className="flex-1 flex flex-col gap-4">
                <div className="space-y-4">
                  ...
                  <label className="flex items-center gap-3 cursor-pointer group">
                    ...
                    <span className="text-xs text-slate-400 group-hover:text-slate-200 transition-colors">Test High Risk Detection</span>
                  </label>
                </div>
        ...
        <div className="lg:col-span-3">
          <AnimatePresence mode="wait">
            {result ? (
              ...
              >
                {/* Result Header */}
                <div className={`glass p-8 border-l-4 ${result.fraud_prediction.is_fraud ? 'border-l-fraud' : 'border-l-safe'}`}>
                  <div className="flex justify-between items-start">
                    <div className="flex gap-4">
                      <div className={`p-4 rounded-2xl ${result.fraud_prediction.is_fraud ? 'bg-fraud/10 text-fraud' : 'bg-safe/10 text-safe'}`}>
                        {result.fraud_prediction.is_fraud ? <ShieldAlert className="w-10 h-10" /> : <ShieldCheck className="w-10 h-10" />}
                      </div>
                      <div>
                        <h2 className="text-2xl font-bold">
                          {result.fraud_prediction.is_fraud ? 'Potential Fraud Detected' : 'Transaction Validated'}
                        </h2>
                        <p className="text-slate-400 text-sm mt-1">
                          Ref ID: <span className="font-mono text-xs">#{result.transaction_id.toString().padStart(6, '0')}</span>
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] text-slate-400 uppercase tracking-widest font-bold mb-1">Risk Score</p>
                      <p className={`text-3xl font-black ${result.fraud_prediction.fraud_probability > 50 ? 'text-fraud' : 'text-safe'}`}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-8">
                    {result.fraud_prediction.insights.map((insight, i) => (
                      <span key={i} className="px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[10px] font-medium text-slate-300">
                        {insight}
                      </span>
                    ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="glass p-6">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <Layers className="w-3 h-3 text-primary" /> Feature Importance
                    </h4>
                    <div className="space-y-3">
                      {Object.entries(result.feature_importance).map(([feature, value], i) => (
                  ...
                  <div className="glass p-6">
                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                      <ChevronRight className="w-3 h-3 text-secondary" /> Behavioral Baseline
                    </h4>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-slate-400">Avg Transaction</span>
                        <span className="text-sm font-bold">${result.behavior_analysis.avg_amount.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-slate-400">Historical Count</span>
                        <span className="text-sm font-bold">{result.behavior_analysis.transaction_count}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-slate-400">User Risk Level</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                          result.behavior_analysis.risk_level === 'Low' ? 'bg-safe/20 text-safe' : 'bg-fraud/20 text-fraud'
                        }`}>
              ...
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center glass border-dashed opacity-50">
                ...
                <p className="text-slate-500 font-medium">Ready to simulate. Click the button above.</p>
              </div>
            )}
          </AnimatePresence>
        </div>
