    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 mt-1">Audit trail for all incoming financial activities</p>
        </div>
      ...
      <div className="glass overflow-hidden">
              ...
              {loading ? (
                Array(5).fill(0).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td colSpan="7" className="px-6 py-6 h-16 bg-white/5 opacity-20"></td>
                  </tr>
                ))
                ...
                <tr key={tx.id} className="hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                      tx.is_fraud ? 'bg-fraud/10 text-fraud' : 'bg-safe/10 text-safe'
                    }`}>
                      {tx.is_fraud ? 'FLAGGED' : 'CLEARED'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-xs font-mono">#{tx.id.toString().padStart(6, '0')}</span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-sm font-bold text-slate-200">${tx.amount.toFixed(2)}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className="w-12 bg-white/10 h-1.5 rounded-full overflow-hidden">
                        <div className={`h-full ${tx.fraud_probability > 50 ? 'bg-fraud' : 'bg-safe'}`} style={{ width: `${tx.fraud_probability}%` }}></div>
                      </div>
                      <span className="text-xs font-mono">{tx.fraud_probability.toFixed(1)}%</span>
                    </div>
                  </td>
        ...
        <div className="p-4 border-t border-white/10 flex justify-between items-center bg-white/5">
          <p className="text-[10px] text-slate-500">Showing page {page} of {totalPages}</p>
